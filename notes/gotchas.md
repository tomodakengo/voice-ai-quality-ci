...
## 17.【自戒】「崖」の数字は単発測定で、5回回したら結論が変わった(2026-06-12)

**症状**: ノイズSNR別CERを最初は **seed1本(12345)** で測り、「20dBまで劣化ゼロ→10dBで崖、
order-001 は10dBで71%(パソコンB化)」と results.md に書いた。QAの癖で「この数字、毎回同じ?
フレークじゃない?」が気になり、**ノイズseedを5本に振り直した**ところ:

- マクロCERの劣化の立ち上がりは **10dB ではなく 20→15dB**(20dB 9.5%, 15dB 18.7%)。
  seed12345 がたまたま20dBで当てていたため、単発では崖が10dBに見えていた。
- 例の **「パソコンB(71%)」は10dBの5本中の最悪引き**。同じ10dBでも seed を変えると
  14〜71% に散る(order-001 10dB: mean 47%±22pt)。15dBも 14〜71%。
- **逆に頑健なセルは完全に決定的**(domain-001 5dB は5本全部11.5%、filler-001 10dB は全部38.1%)。

**原因**: ノイズミキサ(`addNoiseAtSnr`)は seed で波形が変わる。SNR(エネルギー比)が同じでも、
**どの周波数・タイミングにノイズが乗るか**で認識が当たったり外れたりする。単発測定は
「その1枚のノイズ画像」を見ているにすぎない。AmiVoice自体は同一入力に決定的(同じWAVは同じ結果)。

**回避策/教訓**:
- ノイズ等の確率的劣化を入れた回帰テストは、**1セルを多試行**して mean±σ か分位点で見る
  (`npm run measure:snr:robust`。seed数は `--seeds N`)。
- **CER閾値を単発測定で引くと即フレーク**。「同じSNRで2〜3倍ブレる」を踏まえ、閾値は
  ケース別かつ余裕を持たせる(or 中央値ベース)。
- これは音声特有でなく**テスト設計一般の話**(乱数を含むテストは1回緑でも信用しない)。
  QAの基本をそのままASR評価に持ち込んだら当たった、という素直な学び。

> ⚠️ **記事側への申し送り**: PR #11 のドラフトは**単発測定の数字**(「20dBまでゼロ→10dBで崖」
> 「10dB 24.6%」「パソコンB=71%」)で書かれている。事実ではあるが**最悪引きの単発値**。
> B-2(多seed)に差し替えるか、「単発ではこう見えた→多seedで均すとこうだった」という
> **フレーク発見の物語**として書き直すのが等身大かつ正確(後者を推奨)。
>
> **同じ罠がエンジン比較にもあった**: 単発の「10dB 36.5% vs 14.3%(圧勝)」も最も差が開いた引きで、
> 多seedだと 28.7% vs 19.8%・5本中2本は逆転(results.md C-2)。記事の「いちばん持ち帰り価値のある数字」と
> 書いた箇所は「平均的に有利・万能ではない」にトーンダウンが必要。単発で出した強い結論は全部疑う、が教訓。
