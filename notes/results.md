...
**条件**: AmiVoice `-a-general` / HTTP同期 / **6ケース全部** / Node製ノイズミキサ(実SNRは計算値)/ 2026-06-11
**試行**: 24回 / **音声合計**: 91.2秒 / **再生成**: `npm run measure:snr -- --cases order-001,order-002,schedule-001,schedule-002,domain-001,filler-001`
...
**読みどころ**: 20dB までは劣化ゼロ、**10dB で崖**(CER急増)。語による頑健性差も明瞭:
`domain-001`(デシベル/サンプリングレート)は 5dB でも 11.5% と粘る一方、`order-001` は 10dB で崩壊。
`schedule-002` はノイズ非依存で 9.5%(数詞 `十五時`↔`15時` の表記差が主因, 後述)。

> 注: 3ケースのみ(order-001,order-002,filler-001)だと clean 平均 12.7% / 10dB 46.0% / 5dB 48.4%。
> filler-001 のベースライン誤りが効くため、サンプル構成で平均が動く点に注意(件数を必ず併記)。
